...
def run_train_episode(env, agent, rpm, hparams, log_steps):
    ...
    while True:
        ...
        if rpm.size() < hparams['rpm_init_size']:
            ...
            # TODO: check to use EXPL_NOISE * 1.0, or directly 1.0
            action = np.random.normal(action, hparams['explore_noise'] * 1.0)
        ...
        if rpm.size() > hparams['rpm_init_size']:
            ...
            actor_cost, critic_cost = agent.learn(
                batch_obs, batch_action, batch_reward,
                batch_next_obs, batch_terminal)

            tensorboard.add_scalar(
                'train/critic_loss', critic_cost, log_steps + steps)
            if actor_cost is not None:
                tensorboard.add_scalar(
                    'train/actor_loss', actor_cost, log_steps + steps)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train TD3 baseline model for Quadrotor enviroment.')
    parser.add_argument(
